...
//  P&L SNAPSHOT (T – 1 s)
//  ──────────────────────
//  • Fetches the actual fill price from GET /data/trades for this market
...
async function initTrader() {
  ...
  // ── Path B: derive credentials from wallet signature ──────────────────────
    ...
    // IMPORTANT for signatureType=1 (Magic/Google accounts):
    // The derivation client must include signatureType AND funderAddress,
    // otherwise Polymarket returns "Could not create api key".
    // Reference: https://pypi.org/project/py-clob-client/
    ...
    // Try deriveApiKey first (for accounts that already have a key),
    // fall back to createApiKey (for first-time setup).
    try {
    ...
    // Print credentials so user can paste them into .env for next time
  ...
  // ── Build the authenticated trading client ────────────────────────────────
...
async function fetchFillFromTrades(tokenId) {
  ...
  try {
    ...
    // Sort by timestamp descending, take most recent fill
...
// ──────────────────────────────────────────────────────────────────────────────
//  SECTION 10 — P&L SNAPSHOT + POSITION CLOSE  (runs at T − 1 s)
// ──────────────────────────────────────────────────────────────────────────────
 ...
 *
 *  1. Fetch actual fill from /data/trades
 *  2. Read current market price (WS / REST)
 *  3. Calculate unrealised P&L
 *  4. Print P&L card
 *  5. Place a SELL order to close the position before settlement
 */
async function takePnlSnapshot(slug) {
  const bet = getBet(slug);
  if (!bet) return;

  // Use slug-derived end time — secsToExpiry() may already show the next interval
  ...
  // ── 6. Schedule SELL close 30 s after market expiry ──────────────────────
  // BUY fills off-chain immediately, but the on-chain ERC-1155 settlement
  // (Polygon blockchain) takes additional time. We wait 30s before the first
  // check, then poll every 5s until balance is confirmed (up to 100s total).
  const msUntilSell = Math.max((secsLeft + 30) * 1_000, 2_000);
  log.info(`SELL close scheduled in ${Math.round(msUntilSell / 1000)}s (waits for on-chain settlement)`);
  setTimeout(() => closeTrade(slug, { ...bet, fillPrice, sharesFilled }), msUntilSell);

  console.log(chalk.cyan(`  Close     : SELL scheduled ~30s after expiry (awaits settlement)`));
  log.divider();
...
// ──────────────────────────────────────────────────────────────────────────────
//  SECTION 10b — SELL CLOSE  (fires ~10 s after market expiry)
// ──────────────────────────────────────────────────────────────────────────────
//
//  ROOT CAUSE: `createAndPostMarketOrder` is for BUY (dollar-denominated).
//  For SELL you MUST use `createAndPostOrder` (limit order, GTC) with:
//    - side: Side.SELL
//    - price: explicit limit price (tick-size aligned)
//    - size: number of shares (not dollars)
//    - feeRateBps: 0  (limit orders use 0, not 1000)
//
//  Reference: https://github.com/Polymarket/magic-proxy-builder-example
//  The official Polymarket proxy example always uses createAndPostOrder (GTC)
//  for SELL, never createAndPostMarketOrder.
//
//  We also call updateBalanceAllowance(CONDITIONAL, tokenId) first to flush
//  the CLOB's stale balance cache after a BUY fills.

/**
 * Place a GTC limit SELL order to close the position after expiry.
 * Sets price slightly below best bid to ensure fast fill.
 * Retries up to 3 times at 15s intervals.
 *
 * @param {string} slug   — market slug (for logging)
 * @param {object} bet    — the recorded bet from the ledger
 * @param {number} retry  — retry count (max 3)
 */
async function closeTrade(slug, bet, retry = 0) {
  const MAX_RETRIES = 3;

  if (CFG.dryRun) {
    log.dry(`[SIMULATED SELL CLOSE] ${slug}  ${bet.sharesFilled?.toFixed(4)} shares`);
    return;
  }

  log.divider();
  log.trade(`🔄  SELL CLOSE  ${slug}  (attempt ${retry + 1}/${MAX_RETRIES + 1})…`);

  if (retry > MAX_RETRIES) {
    log.warn(`Sell close: gave up after ${MAX_RETRIES + 1} attempts for ${slug}`);
    log.warn('  → Manually sell on polymarket.com to convert equity to cash');
    return;
  }

  const client = await initTrader();

  // ── Step 1: wait until on-chain conditional token balance > 0 ─────────────
  // The BUY fills off-chain immediately, but the on-chain ERC-1155 token
  // transfer (the actual blockchain settlement) takes time.
  // updateBalanceAllowance reads on-chain — if settlement is pending it shows 0.
  // We must wait until the balance is confirmed before placing a SELL.
  log.info('  Waiting for on-chain token settlement…');
  let onChainBalance = 0;
  for (let i = 0; i < 20; i++) {  // poll up to 20 × 5s = 100s max
    try {
      ...
      onChainBalance = parseFloat(bal?.balance ?? '0');
      log.info(`  On-chain balance check ${i + 1}/20: ${onChainBalance} tokens`);
      if (onChainBalance > 0) {
        log.ok(`  Tokens confirmed on-chain: ${onChainBalance}`);
        break;
      }
    } catch (err) {
      log.warn(`  Balance check failed: ${err.message}`);
    }
    if (i < 19) await new Promise(r => setTimeout(r, 5_000));  // wait 5s between checks
  }

  if (onChainBalance === 0) {
    log.warn('  On-chain balance still 0 after 100s — settlement may be delayed.');
    log.warn('  Attempting SELL anyway…');
  }

  // ── Step 2: get sell price ────────────────────────────────────────────────
  // After market expiry, REST /price may return stale values.
  // Use WS cached price first, then REST, then fallback to fillPrice.
  let sellPrice = wsMonitor.getPrice(bet.tokenId);
  if (!sellPrice) {
    try {
      const { data } = await axios.get(`${CFG.clobApiUrl}/price`, {
        params: { token_id: bet.tokenId, side: 'SELL' },
        timeout: 6_000,
      });
      sellPrice = data?.price ? parseFloat(data.price) : null;
    } catch { sellPrice = null; }
  }
  if (!sellPrice) sellPrice = bet.fillPrice;

  // Safety: if price looks wrong (< 0.05 for a winning position), use fill price
  // A token we bought at 80-90% should sell for similar or higher, not 1¢
  if (sellPrice < 0.05 && bet.fillPrice > 0.5) {
    log.warn(`  Post-expiry sell price ${pct(sellPrice)} looks wrong — using fill price ${pct(bet.fillPrice)} instead`);
  ...
  try {
    // Use createAndPostOrder (limit GTC) — NOT createAndPostMarketOrder (FOK)
    // The official Polymarket proxy/Magic example always uses this for SELL.
    const resp = await client.createAndPostOrder(
      {
        tokenID:    bet.tokenId,
        price:      parseFloat(priceStr),   // explicit limit price (tick-aligned)
        size:       sharesRounded,           // number of shares (NOT dollar amount)
        side:       Side.SELL,
        feeRateBps: 1000,                     // this market requires 1000 bps
        expiration: 0,                       // no expiry (GTC)
    ...
    if (resp?.orderID && resp.success !== false) {
      ...
      log.ok(`    Order is live — will fill when matched. Check polymarket.com for settlement.`);

      if (_ledger.has(slug) && _ledger.get(slug).pnlSnapshot) {
        _ledger.get(slug).pnlSnapshot.closeResult = { proceeds, pnl, orderId: resp.orderID };
      }
      return;
    }
    ...
    if (retry < MAX_RETRIES) {
      ...
      setTimeout(() => closeTrade(slug, bet, retry + 1), 15_000);
    } else {
      log.warn('  → Manually sell on polymarket.com');
    }
...
// Key = currentIntervalKey().  We bet AT MOST ONCE per 5-min window across
// ALL three assets combined.
...
async function evaluateAsset(asset, timeLeft) {
  ...
  // Schedule P&L snapshot if we already have a bet on this slug
  if (!_pnlScheduled.has(slug) && hasBet(slug)) {
    const msUntil = Math.max((timeLeft - 1) * 1_000, 0);
    _pnlScheduled.add(slug);
    if (msUntil > 500) {
      log.info(`P&L snapshot for ${asset.label} in ${(msUntil / 1000).toFixed(1)}s`);
      setTimeout(() => takePnlSnapshot(slug), msUntil);
    } else {
      await takePnlSnapshot(slug);
    }
  }
...
async function poll() {
  ...
  // ── 1. Always check P&L snapshots for any existing bets, regardless of window ─
  ...
  if (_bettedIntervals.has(intervalKey)) {
    // Find which slug we bet and report it
  ...
  for (const asset of CFG.assets) {
    ...
    if (result) {
      ...
      break; // First match wins; lower-priority assets not checked
    }
  }
  ...
  if (CFG.dryRun) {
    ...
    if (!result.success) {
      ...
      return; // Don't record — allow retry if time remains
    }
  ...
  recordBet(slug, {
    ...
    sharesFilled,
    tokenId,
    conditionId: signal.meta.conditionId || null,  // needed for redemption
  ...
  // ── 8. Schedule P&L snapshot at T − 1 s ──────────────────────────────────
  if (!_pnlScheduled.has(slug)) {
    const msUntilSnapshot = Math.max((timeLeft - 1) * 1_000, 0);
    _pnlScheduled.add(slug);
    if (msUntilSnapshot > 500) {
      log.info(`P&L snapshot in ${(msUntilSnapshot / 1000).toFixed(1)}s`);
      setTimeout(() => takePnlSnapshot(slug), msUntilSnapshot);
    } else {
      await takePnlSnapshot(slug);
    }
  }
...
function printSessionSummary() {
  ...
  for (const b of bets) {
    ...
    if (snap) {
      ...
      console.log(`    Settle   : ${snap.settlementNote || 'auto-settle at expiry'}`);
